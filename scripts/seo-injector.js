...
const routes = [
  ...
  { 
    path: '/blog/rover-battery', 
    title: 'Rover Battery Build: Powering Autonomous Robotics' + TITLE_SUFFIX, 
    desc: 'Building a Custom 3S2P Lithium Battery Pack for My Smart Agricultural Rover',
    type: 'article'
  },
  { 
    path: '/blog/ieee-research', 
    title: 'IEEE Research Journey: Optimizing Edge Computing' + TITLE_SUFFIX, 
    desc: 'Lessons from Publishing My First IEEE Research Paper as a Student',
    type: 'article'
  },
  { 
    path: '/blog/digsafe', 
    title: 'DIGSAFE: Smart Helmet for Miners' + TITLE_SUFFIX, 
    desc: 'DIGSAFE: A 5G-Enabled Smart Helmet for Worker Safety in Hazardous Environments',
    type: 'article'
  },
  { 
    path: '/blog/smartaid', 
    title: 'SmartAid: Award-Winning Emergency Healthcare System' + TITLE_SUFFIX, 
    desc: 'SmartAid – AI & IoT Powered Emergency Response System',
    type: 'article'
  },
  { 
    path: '/blog/trusttrade', 
    title: 'TrustTrade: Secure Escrow Platform for Digital Assets' + TITLE_SUFFIX, 
    desc: 'How the Idea of TrustTrade Was Born',
    type: 'article'
  },
...
routes.forEach(route => {
  ...
  // Assuming <head> is in the baseHtml
  let newHtml = baseHtml.replace('</head>', `${metaTags}</head>`);
  
  // Replace the default title tag if it exists (to prevent duplicates)
  newHtml = newHtml.replace(/<title>.*?<\/title>/, '');
  newHtml = newHtml.replace(/<meta name="description".*?>/, '');

  // Ensure output directory exists
  const routeDir = path.join(buildDir, route.path === '/' ? '' : route.path);
